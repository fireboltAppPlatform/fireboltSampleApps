...
static gboolean busCallback(GstBus *bus, GstMessage *message, gpointer data)
{
   ...
   switch ( GST_MESSAGE_TYPE(message) ) 
   {
     ...
     case GST_MESSAGE_BUFFERING:
         {
           gint percent;
           static bool triggerPlay = false;
           gst_message_parse_buffering (message, &percent);
           if(percent >= 50 && !triggerPlay)
           {
             triggerPlay = true;
             printf("50 percent buffered, going to play!\n");
...
bool createPipeline( AppCtx *ctx )
{
   ...
   int argc= 0;
   char **argv= 0;
   gint64 bufferDuration =  10 * GST_SECOND;
   ...
   g_object_set(G_OBJECT(ctx->player),"buffer-duration",bufferDuration,NULL);
...
int main( int argc, char **argv )
{
   int result= -1;
   int argidx;
   const char *uri= "http://download.blender.org/peach/bigbuckbunny_movies/big_buck_bunny_720p_h264.mov";
   AppCtx *ctx= 0;
...
*/
   ...
   if ( createPipeline( ctx ) )
   {
      ...
      if ( ctx->loop )
      {
         ...
         if ( GST_STATE_CHANGE_FAILURE != gst_element_set_state(ctx->pipeline, GST_STATE_PAUSED) )
         {
